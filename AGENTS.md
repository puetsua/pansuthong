...
- Views are **queries over tasks + tags** computed in `Document` helpers, not separate stored collections — e.g. `tasks_today` (date-based), `tasks_inbox` (`task_in_inbox` = task has **no pinned tag**, so it isn't surfaced by any pinned-tag sidebar view; untagged tasks and tasks tagged only with unpinned tags both qualify), and `tasks_for_tag`. Task lists are ordered by `settings.sort_order` (`"priority"` = weight desc → date → insertion, the default; or `"date"`), applied in the TS `buildIndexes`.
...
- **Updating via the in-app prompt preserves the taskbar pin and shortcuts; manually re-running setup.exe historically did not.** Tauri's NSIS template only deletes/unpins shortcuts when the uninstaller runs without `/UPDATE`; the in-app updater passes `/UPDATE`, a manual reinstall does not. `src-tauri/windows/hooks.nsh` (wired via `bundle.windows.nsis.installerHooks`) closes that gap by detecting an in-place reinstall (`$EXEDIR == $INSTDIR`) and forcing update mode so the pin/shortcuts survive a manual reinstall too. Because hooks live in the *uninstaller*, the fix only applies when upgrading **from** a build that already ships it. Verify after any installer change with a real Windows build → install → pin to taskbar → bump version → manual reinstall → confirm the pin survives.
