...
Always **identify the filer** in the issue body so a human reading the tracker knows it was filed by an automated agent, not hand-written. Append the attribution footer (see template) naming the app (Codex) and the model. Use the model ID from your environment context.
...
---
🤖 Filed by Codex (model: <model-id, e.g. Codex-opus-4-8>) on behalf of @<github-user>.
```
