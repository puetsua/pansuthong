...
- **Notify the view before the store refresh drops the row.** `TaskEditor` lives inside `TaskRow`. Inbox/Upcoming/Tag/Search (and Today edge cases) remove a completed task on `store-changed` unless `onCompleted` holds it. The editor must call the same `onCompleted` as the row checkbox, and Today must use `useHeldCompletions` too. Otherwise the row unmounts and takes the modal with it. The held set clears when the view unmounts (user switches views).
- **Do not call `onReopened` from the editor.** Dropping the hold while `props.task` is still done makes the row vanish for one frame and closes the modal. The held resolver already ignores ids that are no longer done after the store refresh.
