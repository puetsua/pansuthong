...
- **Notify the view before the store refresh drops the row.** `TaskEditor` lives inside `TaskRow`. Inbox/Upcoming/Tag/Search (and Today edge cases) remove a completed task on `store-changed` unless `onCompleted` holds it. The editor must call the same `onCompleted` / `onReopened` as the row checkbox, and Today must use `useHeldCompletions` too. Otherwise the row unmounts and takes the modal with it. The held set clears when the view unmounts (user switches views).
