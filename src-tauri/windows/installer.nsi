; ============================================================================
; VENDORED + PATCHED copy of Tauri's stock NSIS installer template.
;
; Source: tauri-apps/tauri @ tag tauri-cli-v2.11.2
;   crates/tauri-bundler/src/bundle/windows/nsis/installer.nsi
; Wired in via bundle.windows.nsis.template in tauri.conf.json.
;
; Why we fork it
; --------------
; The stock template shows an "Already Installed" page (PageReinstall) whenever
; a previous install is detected during a *manual* setup.exe run, offering
; "Uninstall before installing" / "Do not uninstall". Picking uninstall removes
; Start Menu/desktop shortcuts and the taskbar pin. We want a manual reinstall to
...
Function PageReinstall
  ...
  ; the taskbar pin. The ${Else} dialog block is kept (now dead) to minimise the
  ; diff against upstream on re-sync.
  ${If} $PassiveMode = 0
  ${OrIf} $PassiveMode = 1
...
Function PageLeaveReinstall
  ...
  ; (handled above) still uninstalls the old .msi. The version-based radio logic
  ; below is now dead for the non-WiX case; kept to minimise the upstream diff.
  Goto reinst_done
