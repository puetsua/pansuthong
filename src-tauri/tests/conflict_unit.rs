...
fn apply_keep_mine_preserves_my_version() {
    ...
    let merged = apply_decisions(&mine, &theirs, &[Decision::KeepMine { id: "a".into() }]);
...
fn apply_keep_theirs_overwrites_mine() {
    let mine = doc(vec![mk("a", "Mine", false)]);
    let theirs = doc(vec![mk("a", "Theirs", false)]);
    let merged = apply_decisions(&mine, &theirs, &[Decision::KeepTheirs { id: "a".into() }]);
    assert_eq!(merged.len(), 1);
...
fn apply_keep_both_clones_theirs_with_new_id() {
    let mine = doc(vec![mk("a", "Mine", false)]);
    let theirs = doc(vec![mk("a", "Theirs", false)]);
    let merged = apply_decisions(&mine, &theirs, &[Decision::KeepBoth { id: "a".into() }]);
...
fn apply_drop_removes_task() {
    ...
    let theirs = doc(vec![]);
    let merged = apply_decisions(&mine, &theirs, &[Decision::Drop { id: "a".into() }]);
    assert_eq!(merged.len(), 1);
...
fn apply_only_theirs_keep_theirs_imports() {
    ...
    let merged = apply_decisions(&mine, &theirs, &[Decision::KeepTheirs { id: "c".into() }]);
    assert_eq!(merged.len(), 2);
