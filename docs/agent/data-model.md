...
- Legacy `tasks_<device>.json` / `tasks.json` are imported on first launch and left in place as a downgrade fallback. Desktop peers merge sibling `tasks_*.db` (and legacy `tasks_*.json`) read-only with a `quick_check` guard; the Android SAF layer (`safsync.rs`) still exchanges JSON until converted to `.db` (see issue #120). Cross-device change detection hashes the decoded document (content), not file bytes, because two `.db` files with identical content are not byte-identical. Merge stays entity-level last-write-wins + tombstones in `model.rs`.
